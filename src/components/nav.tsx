...
export function Nav({ initialDark = true }: { initialDark?: boolean }) {
  ...
  return (
    <>
      ...
      >
        <div className="wrap flex items-center justify-between gap-8 py-3.5">
          <a
            href="#"
            className="inline-flex items-baseline gap-1 text-[26px] font-bold tracking-tight"
          >
            Etax
            <span className="w-1.5 h-1.5 bg-primary -translate-y-1" />
          </a>
...
function MobileMenu({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-40 bg-navy text-paper flex flex-col p-6 pt-24 overflow-y-auto"
    >
      <nav className="flex flex-col">
        {LINK_KEYS.map((key, i) => (
          <motion.a
            ...
            className="flex items-center justify-between py-4 border-b border-paper/10 text-3xl sm:text-4xl font-semibold tracking-tight"
          >
            <span>{t(`links.${key}`)}</span>
            <span className="text-xs opacity-50 font-normal">0{i + 1}</span>
          </motion.a>
        ))}
      </nav>
      <motion.div
      ...
      >
        <p className="text-sm text-paper/60 mb-1">Toshkent · Amir Temur 107A</p>
        <strong className="text-lg font-medium">hello@etax.uz</strong>
      </motion.div>
    </motion.div>
  );
}
